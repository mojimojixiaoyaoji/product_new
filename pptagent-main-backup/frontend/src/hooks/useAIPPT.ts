...
export default () => {
  ...
  // 模板使用次数追踪，用于负载均衡
  const templateUsageCount = ref<Map<string, number>>(new Map())
  const lastContentTemplateId = ref<string | null>(null)

  /**
   * 选择使用次数最少的模板
   * 如果多个模板使用次数相同，则随机选择其中一个
   * @param templates 候选模板列表
   * @returns 选中的模板
   */
  const selectLeastUsedTemplate = (templates: Slide[]): Slide | undefined => {
    if (!templates.length) return undefined

    // 找出最小使用次数
    let minUsage = Infinity
    for (const template of templates) {
      const templateId = template.id || template.type || String(Math.random())
      const usage = templateUsageCount.value.get(templateId) || 0
      if (usage < minUsage) {
        minUsage = usage
      }
    }
    ...
    const candidates = templates.filter(template => {
      const templateId = template.id || template.type || String(Math.random())
      const usage = templateUsageCount.value.get(templateId) || 0
      return usage === minUsage && (!lastContentTemplateId.value || templateId !== lastContentTemplateId.value)
    })
    const leastUsedTemplates = candidates.length ? candidates : templates.filter(template => {
      const templateId = template.id || template.type || String(Math.random())
      const usage = templateUsageCount.value.get(templateId) || 0
    ...
    // 从使用次数最少的模板中随机选择一个
    const selected = leastUsedTemplates[Math.floor(Math.random() * leastUsedTemplates.length)]

    // 更新使用次数
    const templateId = selected.id || selected.type || String(Math.random())
    const currentCount = templateUsageCount.value.get(templateId) || 0
    templateUsageCount.value.set(templateId, currentCount + 1)
  ...
  function* AIPPTGenerator(templateSlides: Slide[], _AISlides: AIPPTSlide[], imgs?: ImgPoolItem[]) {
    ...
    if (!transitionTemplate.value) {
      const _transitionTemplate = selectLeastUsedTemplate(transitionTemplates)
      transitionTemplate.value = _transitionTemplate || null
    }
    ...
    for (const item of AISlides) {
      // 封面页处理
      if (item.type === 'cover') {
        const coverTemplate = selectLeastUsedTemplate(coverTemplates)
      ...
      // 目录页处理
        ...
        const contentsTemplate = selectLeastUsedTemplate(_contentsTemplates)
        if (!contentsTemplate) continue
      ...
      // 过渡页处理
        ...
        const contentTemplate = selectLeastUsedTemplate(_contentTemplates)
        if (contentTemplate) lastContentTemplateId.value = contentTemplate.id || contentTemplate.type || null
        ...
        const referenceTemplate = selectLeastUsedTemplate(_referenceTemplates)
        if (!referenceTemplate) continue
      ...
      // 结束页处理
        ...
        const endTemplate = selectLeastUsedTemplate(endTemplates)
        if (!endTemplate) continue
  ...
  const AIPPT = (templateSlides: Slide[], _AISlides: AIPPTSlide[], imgs?: ImgPoolItem[]) => {
    // 生成所有幻灯片
    const slides: Slide[] = [...AIPPTGenerator(templateSlides, _AISlides, imgs)].filter((s): s is Slide => !!s)
