    ...
    <div class="aippt-dialog">
      ...
      <header class="header" role="banner">
        <div class="header-content">
          <img src="/logo-agent.png" alt="PPT智能体" class="header-logo" />
          <h1 class="title">PPT智能体</h1>
    ...
    const processEvent = (evt: string) => {
      ...
      try {
        ...
        for (const generatedSlide of slideGenerator) {
          if (!generatedSlide.elements) continue
          if (isEmptySlide.value) {
            slideStore.setSlides([generatedSlide as Slide])
          } else {
            addSlidesFromDataToEnd([generatedSlide as Slide])
          }
        }
      } catch (e) {
...
.page-bg {
  ...
  inset: 0;
  z-index: 0;
  background: radial-gradient(1200px 600px at 10% -10%, rgba(215, 0, 15, 0.12), rgba(0, 0, 0, 0) 60%),
    radial-gradient(1000px 600px at 90% 110%, rgba(215, 0, 15, 0.12), rgba(0, 0, 0, 0) 60%),
...
.bg-blob.b1 { width: 520px; height: 520px; left: -160px; top: -160px; background: #ffb3b3; }
.bg-blob.b2 { width: 420px; height: 420px; right: -120px; bottom: -120px; background: #ffcccc; }
...
.select-template {
  ...
  .template-card {
    ...
    &.selected {
      border-color: #D7000F;
      box-shadow: 0 0 0 3px rgba(215, 0, 15, 0.15), 0 12px 28px -6px rgba(215, 0, 15, 0.25);
      .overlay { opacity: 1; visibility: visible; }
      .template-info { background: linear-gradient(135deg, #D7000F 0%, #D7000F 100%); color: #fff; }
    }
    ...
    .template-image {
      ...
      .overlay {
        position: absolute; inset: 0; background: rgba(215, 0, 15, 0.18);
        display: flex; align-items: center; justify-content: center;
        opacity: 0; visibility: hidden; transition: all 0.25s ease;
        .check-icon {
          width: 32px; height: 32px; color: #fff; background: #D7000F; border-radius: 50%;
          display: flex; align-items: center; justify-content: center; box-shadow: 0 4px 12px rgba(215, 0, 15, 0.35);
  ...
  .actions {
    ...
    .btn {
      ...
      &.btn-primary {
        background: linear-gradient(135deg, #D7000F 0%, #D7000F 100%); border: none; color: #fff;
        box-shadow: 0 6px 16px rgba(215, 0, 15, 0.38);
        &:hover:not(:disabled) { transform: translateY(-1px); box-shadow: 0 10px 24px rgba(215, 0, 15, 0.5); .btn-icon { transform: rotate(90deg); } }
