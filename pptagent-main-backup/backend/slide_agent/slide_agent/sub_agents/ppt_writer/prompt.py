...
COVER_PAGE_PROMPT="""
封面页（type: "cover"）
你是PPT封面文案优化器,使用的语言是{language}。保持 title 原样，不改；必须重写 data.text 为 18～32 字的副标题，强调主题价值与适用场景，避免标点堆叠与口号化。如果 data.text 为空或不存在，必须生成一个合适的副标题。
{input_slide_data}
"""
...
过渡页（type: "transition"）
你是章节过渡文案撰写者,使用的语言是{language}。保持 data.title 原样不改；必须重写 data.text 为2～3句过渡语，每句12～24字，说明本章为何重要、将回答什么问题、读者可获得的收获。避免夸张或口号化表达。如果 data.text 为空或不存在，必须生成一个合适的过渡语。
{input_slide_data}
"""
...
# 关键指令：
- **data.title 是页面的主标题**，例如："大语言模型的进化"、"生成式AI的商业应用" 等。**必须原样保留，不得修改为"标题"或任何其他值**。
- **每个 items[*].title 是条目的小标题**，例如："多模态融合" 等。**必须原样保留，不得修改**。
- 对 items[*].text 逐项扩写为 2～3 句、合计 60～120 字，采用"是什么→为何重要→如何落地/示例"的逻辑。

# 禁止事项：
- 不得删除已有的 items
- 不得修改 data.title 的值（禁止改为"标题"或其他值）
- 不得修改 items[*].title 的值
- 不得保留或复述 items[*].text 中的占位符原文（如"详细内容待补充"、"Detailed content about"等）
- 不得输出任何未填充的占位符
- 避免增加 data.items 的子项
- 避免编造精确数据或过度承诺
...
# 原始输入数据（data.title 和 items[*].title 必须保持原样）：
{input_slide_data}
"""
else:
    CONTENT_PAGE_PROMPT = """
