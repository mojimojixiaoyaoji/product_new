...
# 控制是否开启 LiteLLM 的 debug 模式，默认关闭
LITELLM_DEBUG = os.getenv("LITELLM_DEBUG", "false").lower() in ("true", "1", "yes", "on")
if LITELLM_DEBUG:
    litellm._turn_on_debug()

# 控制是否启用 LiteLLM 的计费功能，默认关闭
# 关闭后可以避免 "This model isn't mapped yet" 的错误信息
LITELLM_BILLING_ENABLED = os.getenv("LITELLM_BILLING_ENABLED", "false").lower() in ("true", "1", "yes", "on")
if not LITELLM_BILLING_ENABLED:
    # 禁用计费功能，避免获取模型价格信息时的错误
    litellm.drop_params = True
    # 设置为空字典以禁用价格和上下文窗口的获取
    litellm.model_prices = {}
    litellm.model_context_window = {}
def create_model(model:str, provider: str):
    ...
    if provider == "google":
        ...
        if not model.startswith("openai/"):
            # 表示兼容openai的模型请求
            model = "openai/" + model
        return LiteLlm(model=model, api_key=os.environ.get("DEEPSEEK_API_KEY"), api_base="https://ilm-prod.allintechinc.com/model-qwen3-next-service/v1",num_retries=3)
    elif provider == "glm":
        # GLM的模型需要使用LiteLlm
